...
void handlelevels(Clock&, RenderWindow&);
...
void loader()
{
	...
	skull.setTexture(*gametile);
	skull.setTextureRect(IntRect(246, 54, 42, 57));
	skull.setOrigin(42/2, 57/2);
...
void handlelevels(Clock& clock, RenderWindow& window)
{
	///////////////////////
	///////////////////////
	...
	for (int k = 0; k < frog.size(); k++)
	{
		...
		{
			skull.setPosition(*frog[k].returnpos());

			//skullclock.restart();

			//window.draw(skull);
			
		//	if (skullclock.getElapsedTime().asSeconds()>15)
			//{
				//skullclock.restart();
	...
	if (clock.getElapsedTime().asSeconds() > .1)
	{
		...
		for (int j = 0; j < enemy[0].size(); j++)
			enemy[0][j].speed(scalefactor->x*clock.getElapsedTime().asSeconds() * speed[1][j] * enemyspeedfactor);

		for (int j = 0; j < enemy[1].size(); j++)
			enemy[1][j].speed(scalefactor->x*clock.getElapsedTime().asSeconds() * speed[1][3+j] * enemyspeedfactor);

		for (int j = 0; j < enemy[2].size(); j++)
			enemy[2][j].speed(scalefactor->x*clock.getElapsedTime().asSeconds() * speed[1][6+ j] * enemyspeedfactor);

		for (int j = 0; j < enemy[3].size(); j++)
			enemy[3][j].speed(scalefactor->x*clock.getElapsedTime().asSeconds() * speed[1][9 + j] * enemyspeedfactor);

		for (int j = 0; j < enemy[4].size(); j++)
			enemy[4][j].speed(scalefactor->x*clock.getElapsedTime().asSeconds() * speed[1][11 + j] * enemyspeedfactor);

		for (int j = 0; j < enemy[5].size(); j++)
			enemy[5][j].speed(scalefactor->x*clock.getElapsedTime().asSeconds() * speed[1][13 + j] * enemyspeedfactor);

		for (int j = 0; j < enemy[6].size(); j++)
			enemy[6][j].speed(scalefactor->x*clock.getElapsedTime().asSeconds() * speed[1][15 + j] * enemyspeedfactor);
		...
		for (int j = 0; j < helper[0].size(); j++)
			helper[0][j].speed(scalefactor->x*clock.getElapsedTime().asSeconds() * speed[2][j] * helperspeedfactor);

		for (int j = 0; j < helper[1].size(); j++)
			helper[1][j].speed(scalefactor->x*clock.getElapsedTime().asSeconds() * speed[2][1 + j] * helperspeedfactor);

		for (int j = 0; j < helper[2].size(); j++)
			helper[2][j].speed(scalefactor->x*clock.getElapsedTime().asSeconds() * speed[2][3 + j] * helperspeedfactor);
...
*/
	...
	*/
	...
	skull.scale(scalefactor->x, scalefactor->y);
